...
from utils.parser import parse_markdown_to_workflow
from utils.llm import retrieve_references, invoke_completion

from agent.list_gen_agent.utils.state import AgentState
from agent.list_gen_agent.utils.function import fetch_reference_by_name
from agent.list_gen_agent.agent.analyzer import get_analyzer_agent_prompt
from agent.list_gen_agent.agent.planner import (
    get_planner_agent_prompt,
    parse_planner_agent_response,
    parse_planner_agent_action
)
from agent.list_gen_agent.agent.combiner import (
    get_combiner_agent_prompt,
    parse_combiner_agent_response
)
from agent.list_gen_agent.agent.adapter import (
    get_adapter_agent_prompt,
    parse_adapter_agent_response
)
from agent.list_gen_agent.agent.refiner import (
    get_refiner_agent_prompt,
    parse_refiner_agent_response
)


class ListGenAgentPipeline:
    def __init__(
        self,
        save_path: str,
        num_steps: int = 5,
    ...
    def __call__(self, query_text: str):
        ...
        with open(path['markdown'], 'r') as markdown_file:
            workspace['markdown'] = markdown_file.read()
        ...
        for step_epoch in range(1, self.num_steps + 1):
            ...
            if command == 'load':
                ...
                state.update_workspace(markdown, description)
            ...
            # Action `combine`
                ...
                markdown, description = parse_combiner_agent_response(combiner_response)
                self.logger.info(f'Parsed markdown:\n  {markdown}')
            ...
            # Action `adapt`
                ...
                markdown, description = parse_adapter_agent_response(adapter_response)
                self.logger.info(f'Parsed markdown:\n  {markdown}')
                self.logger.info(f'Parsed description:\n  {description}')
            ...
            for fix_epoch in range(self.num_fixes + 1):
                try:
                    markdown = state.workspace['markdown']
                    ...
                    explanation, markdown, description = parse_refiner_agent_response(refiner_response)
            ...
            with open(f'{self.save_path}/markdown.md', 'w') as markdown_file:
