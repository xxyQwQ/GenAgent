...
from agent.code_gen_agent.utils.function import fetch_reference_by_name
from agent.code_gen_agent.agent.analyzer import get_analyzer_agent_prompt
from agent.code_gen_agent.agent.planner import (
    get_planner_agent_prompt,
    parse_planner_agent_response,
    parse_planner_agent_action
)
from agent.code_gen_agent.agent.combiner import (
    get_combiner_agent_prompt,
    parse_combiner_agent_response
)
from agent.code_gen_agent.agent.adapter import (
    get_adapter_agent_prompt,
    parse_adapter_agent_response
)
from agent.code_gen_agent.agent.refiner import (
    get_refiner_agent_prompt,
    parse_refiner_agent_response
)


class CodeGenAgentPipeline:
    def __init__(
        self,
        save_path: str,
        num_steps: int = 5,
    ...
    def __call__(self, query_text: str):
        ...
        for step_epoch in range(1, self.num_steps + 1):
            ...
            if command == 'load':
                ...
                state.update_workspace(code, description)
            ...
            # Action `combine`
                ...
                code, description = parse_combiner_agent_response(combiner_response)
            ...
            # Action `adapt`
                ...
                code, description = parse_adapter_agent_response(adapter_response)
                self.logger.info(f'Parsed code:\n  {code}')
                self.logger.info(f'Parsed description:\n  {description}')
            ...
            for fix_epoch in range(self.num_fixes + 1):
                try:
                    ...
                    explanation, code, description = parse_refiner_agent_response(refiner_response)
